...
| Measured unit costs calibrate the operation-count model | P0b/Day 2 plus frozen mixed-circuit correctness gate | A replacement/extension of [`day2-microbench.yml`](../../.github/workflows/day2-microbench.yml): `r3-day2-<experiment-sha>` and `review-pack-R3-Day2-<experiment-sha>` with exactly 14 complete whole measurement blocks, the Day1A-derived exact key plan, all 14 profiles, provenance, checksums, and a named mixed-circuit artifact | HOLD; exploratory run `32712608022` at `f11e97d` verified the raw-block mechanism for all 14 primitives and eight requested rotations, but it contains only 11 measurement blocks, explicitly fails the publication block contract, and lacks the registered key plan, fixed-host/toolchain bindings, canonical R3 archive, evidence anchors, and mixed-circuit artifact |
