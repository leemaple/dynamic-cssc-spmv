...
| Measured unit costs calibrate the operation-count model | P0b/Day 2 plus frozen mixed-circuit correctness gate | A replacement/extension of [`day2-microbench.yml`](../../.github/workflows/day2-microbench.yml): `r3-day2-<experiment-sha>` and `review-pack-R3-Day2-<experiment-sha>` with exactly 14 complete whole measurement blocks, the Day1A-derived exact key plan, all 14 profiles, provenance, checksums, and a named mixed-circuit artifact | HOLD; the historical isolated probe can now emit raw blocks for the closed vocabulary and all requested rotations, but its workflow still uses 11 repeats and lacks the registered key plan, fixed-host/toolchain bindings, canonical R3 archive, evidence anchors, and mixed-circuit artifact |
