...
![图 1：同一个不可变版本把更新、查询重排、Cloud 公开执行与 Client B 私有重构闭合起来。](assets/route-c-protocol-flow.png){width=6.3in}
...
对版本 $v$ 的第 $k$ 个组件，设第 $p$ 个物理 value lane 与并行 `ColumnIndex` 条目中的全局列编号 $g_{v,k,p}$ 成对。Client B 根据该版本的 `ColumnIndex` 构造对齐查询：
